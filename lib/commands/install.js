...
class Install extends ArboristWorkspaceCmd {
  ...
  async exec (args) {
    ...
    // Blacklist gate: refresh the compromised-packages list and refuse any
    // install whose direct args match. Network failures fall back to the
    // last cached or bundled list so we never silently disable the check.
    const allowBlocked = this.npm.config.get('allow-blocked') || forced
    const blacklist = new Blacklist(this.npm)
    await blacklist.load()
    if (!allowBlocked) {
      const directHits = args
        .map(a => blacklist.checkArg(a))
        .filter(Boolean)
      if (directHits.length) {
        throw Blacklist.buildBlockedError(directHits, blacklist.data?.source)
      }
    }
    ...
    // Resolve the full dependency tree first so the blacklist can inspect
    // transitive packages before anything is written to node_modules.
    if (!allowBlocked) {
      await arb.buildIdealTree(opts)
      const treeHits = blacklist.checkTree(arb.idealTree)
      if (treeHits.length) {
        throw Blacklist.buildBlockedError(treeHits, blacklist.data?.source)
      }
    }
