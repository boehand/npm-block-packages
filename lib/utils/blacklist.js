...
const DEFAULT_URL = 'https://raw.githubusercontent.com/boehand/npm-block-packages/blocked-list/blocked.json'
...
class Blacklist {
  ...
  async update ({ force = false, silentErrors = false } = {}) {
    ...
    // Escape hatch for tests and isolated/air-gapped environments: skip the
    // fetch and use an empty in-memory list. Cached and bundled data are
    // ignored so the gate becomes a no-op.
    if (process.env.NPM_BLACKLIST_DISABLED === '1') {
      this.#data = { fetchedAt: now, source: 'disabled', packages: {} }
      return this.#data
    }
    ...
    if (!force && isFresh) {
      this.#data = this.#mergeWithBundled(cached)
      return this.#data
    }
    ...
    try {
      ...
      this.#data = this.#mergeWithBundled(fresh)
      return this.#data
    } catch (err) {
  ...
  // Always overlay the curated bundled defaults onto whatever was loaded
  // remotely. Bundled entries carry hand-written reasons and advisory URLs
  // for famous historical compromises that the upstream aggregator's
  // compact format strips out (and that aren't always in the remote feed
  // at all). Bundled metadata wins; remote-only packages are kept as-is.
  #mergeWithBundled (data) {
    if (!data?.packages) {
      return data
    }
    const merged = { ...data.packages }
    for (const [name, entry] of Object.entries(BUNDLED.packages || {})) {
      merged[name] = entry
    }
    return { ...data, packages: merged }
  }
  ...
  // Normalize a raw blacklist entry. Bundled defaults are objects with
  // `versions/reason/advisory`; large remote lists are compact and store the
  // versions value directly (string, array, or "*") to keep payloads small.
  #normalizeEntry (raw) {
    if (raw == null) {
      return null
    }
  ...
  matchVersion (name, version) {
    ...
    const entry = this.#normalizeEntry(this.#data.packages[name])
    if (!entry) {
      return null
    }
