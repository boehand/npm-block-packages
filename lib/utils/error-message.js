...
const errorMessage = (er, npm) => {
  ...
  switch (er.code) {
    ...
    case 'EBLOCKED':
      summary.push(['blacklist', er.message])
      detail.push(['blacklist', [
        'One or more packages are on the compromised-packages blacklist.',
        'Run `npm blacklist list` to see the current list, or',
        '`npm blacklist update` to refresh it now.',
        'Use `--allow-blocked` to override (not recommended).',
      ].join('\n')])
      break
